...
**Identity parameters** (`--handle`, `--user-mail`, `--user-name`) fall back to their corresponding top-level config keys (`handle`, `user-mail`, `user-name`), not to `defaults`. They are never prompted for.
...
- `/trapper-keeper --handle:_ --user-mail:_ --user-name:_` *(skip all identity checks)*
